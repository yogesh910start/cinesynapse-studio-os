...
tad_content = """# MASTER TECHNICAL ARCHITECTURE DOCUMENT (TAD)
...
* **Mathematical Basis:** When sound recorders run at 24.000 fps and cameras run at 23.976 fps, drift accumulates at:
  $$\Delta t = \left( \frac{24.000 - 23.976}{24.000} \right) \times T_{runtime} = 0.001 \times T_{runtime}$$
...
* **Layout Hierarchy:**
  ...
  * *Central Viewport (Node Graph):* Interactive WebGL force-directed production graph connecting Script Scenes $\rightarrow$ Camera Takes $\rightarrow$ Synthetic Double Passes $\rightarrow$ VFX Renders $\rightarrow$ Territory Masters. Nodes glow purple/cyan when healthy, pulsing amber/red when breached.
...
* **Layout Hierarchy:**
  ...
  * *Center Inspector:* Real-time micro-sentry status cards.
    * Card 1: Fractional Timecode Drift Monitor (Camera: `23.976 fps` vs Master Audio: `24.000 fps` $\rightarrow$ Alert: `+7.2s Drift Detected over 2h` $\rightarrow$ Auto Pull-Up Activated).
    * Card 2: DGA/IATSE Meal Penalty Countdown (`11m 42s Remaining until 2nd Penalty Tier` $\rightarrow$ Exposure: `$3,500.00`).
...
scratch_path = "/Users/ymore/.gemini/antigravity/scratch/cine_synapse/docs/TECHNICAL_ARCHITECTURE_DOCUMENT.md"
brain_path = "/Users/ymore/.gemini/antigravity/brain/fba0b760-63ef-46ec-8f1c-65de7e95e8a1/TECHNICAL_ARCHITECTURE_DOCUMENT.md"
...
print(f"Successfully generated expanded TAD: {len(tad_content)} characters / {len(tad_content.encode('utf-8'))} bytes.")
